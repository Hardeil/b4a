...
Private Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	clrFull = xui.PaintOrColorToColor(Colors.ARGB(255, 16, 64, 34))
	clrEmpty = xui.PaintOrColorToColor(Colors.ARGB(255, 255, 242, 178))
	stroke = DipToCurrent(Props.Get("StrokeWidth")) ' Set from designer property
	' Example of dynamic change:
	stroke = 20dip  ' Set to 20dip dynamically
...
Private Sub DrawValue(Value As Float)
	...
	mLbl.Text = $"$1.0{Value}"$
	Dim startAngle = -90, sweepAngle = Value / (totalValue + .01) * 360 As Float
	If Value < 100 Then
		Dim p As B4XPath
